...
import { requestedbyFemale } from '../../../../actions/reportActions';
...
const MostRequestedByFemale = () => {

    const dispatch = useDispatch();

    useEffect(() => {
        dispatch(getData());
        dispatch(requestedbyFemale());
    }, [dispatch]);

    const { requestedbyfemale } = useSelector(state => state.requestedbyFemales);
    const { loading } = useSelector(state => state.datas);
    let reqsByFemaleData = requestedbyfemale.map(s => s.count);
    let reqsByFemaleName = requestedbyfemale.map(s => s.subject[0].name);
...
export default MostRequestedByFemale
